...
func setupVault(addr, rootToken string) error {
	...
	if _, err := client.Logical().Write(
		"transit/keys/alex2006hw",
		map[string]interface{}{},
	); err != nil {
		return err
	}

	// write alex2006hw policy
	if err := client.Sys().PutPolicy("alex2006hw", alex2006hwPolicyRules); err != nil {
		return err
	}

	// mount approle and write alex2006hw approle
	if err := client.Sys().EnableAuthWithOptions("approle", &api.EnableAuthOptions{
		Type: "approle",
	}); err != nil {
		return err
	}
	if _, err := client.Logical().Write("auth/approle/role/alex2006hw", map[string]interface{}{
		"role_name":          "alex2006hw",
		"policies":           "default, alex2006hw",
	...
	if _, err := client.Logical().Write("auth/approle/role/alex2006hw/role-id", map[string]interface{}{
		"role_id": "alex2006hw",
	}); err != nil {
		return err
	}
	...
	if _, err := client.Logical().Write("secret/alex2006hw", map[string]interface{}{
		"TransitBackend":    "transit",
		"UserTransitKey":    "usertransit",
		"ServerTransitKey":  "alex2006hw",
	...
	if _, err := client.Logical().Write("pki/roles/alex2006hw", map[string]interface{}{
		"allow_any_name":    "true",
	...
	if _, err := client.Logical().Write("pki/issue/alex2006hw", map[string]interface{}{
...
func generateWrappedSecretID(v VaultConfig, token string) (string, error) {
	...
	resp, err := client.Logical().Write("auth/approle/role/alex2006hw/secret-id", map[string]interface{}{})
	if err != nil {
		return "", err
	}
...
const alex2006hwPolicyRules = `# [mandatory]
# store alex2006hw run-time settings here
# alex2006hw hot-reloads from this endpoint every minute
path "secret/alex2006hw*" {
...
path "transit/encrypt/alex2006hw" {
  capabilities = ["read", "update"]
}
path "transit/decrypt/alex2006hw" {
  capabilities = ["read", "update"]
}


# [optional]
# for alex2006hw to fetch certificates from PKI backend
path "pki/issue/alex2006hw" {
  capabilities = ["update"]
}
`
