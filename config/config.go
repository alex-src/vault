...
func LoadConfigDev() (*Config, chan struct{}, []string, string, error) {
	...
	err = setupVault("http://127.0.0.1:8200", "alex2006hw")
	if err != nil {
		return nil, nil, nil, "", err
	}

	// setup alex2006hw internal config
	...
	secretID, err := generateWrappedSecretID(*result.Vault, "alex2006hw")
	if err != nil {
		return nil, nil, nil, "", err
	}
...
func parseVault(result *Config, vault *ast.ObjectItem) error {
	...
	if runtimeConfig, ok := m["runtime_config"]; ok && runtimeConfig != "" {
		...
		result.Vault.Runtime_config = "secret/alex2006hw"
	}
	...
	if id, ok := m["approle_id"]; ok && id != "" {
		...
		result.Vault.Approle_id = "alex2006hw"
	}
